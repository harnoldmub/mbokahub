...
export default async function GamePage({ params, searchParams }: GamePageProps) {
  ...
  const grouped = await prisma.gameScore
    .groupBy({
      by: ["pseudo"],
    ...
    .catch((err) => {
      console.error("[jeu] database unavailable", err);
      return [];
    });
  ...
  const leaderboard = grouped.map((row, i) => {
    ...
    return {
      user: row.pseudo,
      score: formatScore(row._max.score ?? 0, lang),
          ...
          <div className="space-y-6 rounded-[2.5rem] border border-white/10 bg-coal/80 p-8 shadow-2xl backdrop-blur-xl">
            ...
            {lastUpdate && (
              <p className="pt-4 text-center font-mono text-[9px] text-paper-mute uppercase tracking-widest">
                {copy.updatedAt} {formatUpdatedAt(lastUpdate, lang)}
              </p>
            )}
          </div>
        </div>
