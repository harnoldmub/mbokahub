import { Plus } from "lucide-react";
import Link from "next/link";

import { AftersListClient } from "@/components/afters/afters-list-client";
import { Button } from "@/components/ui/button";
import { prisma } from "@/lib/db/prisma";

export const dynamic = "force-dynamic";

const FR_DAYS = [
  "Dimanche",
  "Lundi",
  "Mardi",
  "Mercredi",
  "Jeudi",
  "Vendredi",
  "Samedi",
];
const FR_MONTHS = [
  "janvier",
  "février",
  "mars",
  "avril",
  "mai",
  "juin",
  "juillet",
  "août",
  "septembre",
  "octobre",
  "novembre",
  "décembre",
];

function formatAfterDate(date: Date): string {
  const d = FR_DAYS[date.getUTCDay()];
  const day = date.getUTCDate();
  const m = FR_MONTHS[date.getUTCMonth()];
  const hh = String(date.getUTCHours()).padStart(2, "0");
  const mm = String(date.getUTCMinutes()).padStart(2, "0");
  return `${d} ${day} ${m}, ${hh}:${mm}`;
}

export default async function AftersPage() {
  const today = new Date();
  today.setUTCHours(0, 0, 0, 0);

  // Plateforme 100% gratuite pour les fans : tous les afters validés sont
  // visibles sans paywall. Ordre : boostés d'abord, puis chronologique.
  const aftersDb = await prisma.after
    .findMany({
      where: {
        isApproved: true,
        isActive: true,
        date: { gte: today },
      },
      orderBy: [
        { isBoosted: "desc" },
        { date: "asc" },
        { createdAt: "desc" },
      ],
      take: 200,
    })
    .catch((err) => {
      console.error("[afters] database unavailable", err);
      return [];
    });

  const afters = aftersDb.map((a) => ({
    slug: a.slug,
    name: a.name,
    dateLabel: formatAfterDate(a.date),
    venue: a.venue,
    city: a.city,
    priceFrom: a.priceFrom,
    ticketUrl: a.ticketUrl,
    isBoosted: a.isBoosted,
    flyerUrl: a.flyerUrl,
  }));

  return (
    <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="mb-1 font-mono text-[10px] uppercase tracking-[0.3em] text-ember">
            Soirées post-concert · Gratuit pour la famille
          </p>
          <h1 className="font-display text-6xl uppercase leading-[0.9] text-paper sm:text-7xl">
            Les <span className="font-serif italic text-ember">Afters</span>
          </h1>
          <p className="mt-4 max-w-md font-body text-paper-dim">
            Toutes les soirées validées par le Hub pour fêter l&apos;Aigle après
            le concert.
          </p>
        </div>
        <Button
          asChild
          className="h-14 px-8 border-ember/40 text-ember hover:bg-ember hover:text-paper"
          size="lg"
          variant="outline"
        >
          <Link href="/afters/organiser">
            <Plus aria-hidden className="mr-2 size-4" />
            Proposer un after
          </Link>
        </Button>
      </div>

      <AftersListClient afters={afters} />
    </main>
  );
}
