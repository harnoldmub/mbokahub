...
async function ensureProduct(name, metadata = {}) {
  ...
  if (list.data[0]) {
    console.log(`  ✓ Product exists: ${name} (${list.data[0].id})`);
    return list.data[0];
  }
  const p = await stripe.products.create({ name, metadata });
...
async function ensureWebhook(url) {
  ...
  const w = await stripe.webhookEndpoints.create({
    ...
    description: "Mboka Hub production webhook",
  });
...
(async () => {
  ...
  const vipProduct = await ensureProduct("Mboka Hub · Pass VIP Warrior", { type: "vip" });
  const vipPrice = await ensurePrice(vipProduct.id, "mbokahub_vip_10", 1000, "VIP standard");
  const vipEarlyPrice = await ensurePrice(vipProduct.id, "mbokahub_vip_early_7", 700, "VIP Early Bird");

  const proProduct = await ensureProduct("Mboka Hub · Inscription Pro", { type: "pro" });
  const proPrice = await ensurePrice(proProduct.id, "mbokahub_pro_20", 2000, "Pro premium");

  const boostProduct = await ensureProduct("Mboka Hub · Boost Vedette", { type: "boost" });
  const boostPrice = await ensurePrice(boostProduct.id, "mbokahub_boost_9", 900, "Boost annonce");
